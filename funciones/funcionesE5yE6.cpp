...
bool existeArista(int origen, int destino, Grafo<ciudad *> *g)
{
    ...
    while (adyO)
    {
        if (adyO->destino == destino)
            ret = true;
        adyO = adyO->sig;
    }
    while (adyD)
    {
        if (adyD->destino == origen)
            ret = true;
        adyD = adyD->sig;
    }
...
// PRE: -
// POS: funcion que devuelve el costo de realizar las misiones en un cierto orden
int calcularCosto(int org, int dest, int dest2, int final, nodoLista *&p1, nodoLista *&p2, nodoLista *&p3, Grafo<ciudad *> *c)
{
    ...
    // inicializo vectores y hago dijkstra
    ...
    // construyo el camino de menor costo para ir al primer destino y dubplico el valor de las aristas
    while (voyA != estoyEn)
    {
    ...
    // incremento el acumulador de costos
    ret += costos2[voyA];
