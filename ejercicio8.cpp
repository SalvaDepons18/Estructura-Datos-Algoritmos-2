...
void merge(Ciudad **ciudades, int left, int mid, int right, bool ordenarPorX)
{
    ...
    { // inicializo valores izquierda
        vecIzq[i] = ciudades[left + i];
    }
    for (int j = 0; j < largo2; j++)
    { // inicializo valores derecha
        vecDer[j] = ciudades[mid + 1 + j];
    }

    int i = 0, j = 0, k = left;
    while (i < largo1 && j < largo2)
    {                                                                                               // mientras i y j esten dentro del alcance del vector asociado (izq y der respectivamente)
    ...
    while (i < largo1)
    { // si paro porque se quedo sin j va aca
        ciudades[k] = vecIzq[i];
        i++;
        k++;
    }
    while (j < largo2)
    { // si paro porrque se quedo sin i se va a aca
        ciudades[k] = vecDer[j];
        j++;
        k++;
    }
